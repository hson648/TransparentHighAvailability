...
{
    tree_t *tree = NULL;
    ha_calloc(handle, tree, tree_t, 1);
    if(!tree)
        return NULL;
    tree->root = NULL;
...
tree_node_t* init_tree_node(int n)
{
    ...
    ha_calloc(handle, node, tree_node_t, 1);
    if(!node)   return NULL;
    node->data = n;
    node->isVisited = TREE_FALSE;
    return node;
...
static int _MaxSumPath(tree_node_t *root){
    ...
    int valuetobeReturnedtoParent = MAX(MAX(l + root->data, r + root->data);
                                    MAX(l + root->data + r, root->data));
    if(maxSumPath < valuetobeReturnedtoParent)
        maxSumPath = valuetobeReturnedtoParent;
...
static int _MaxSumPath(tree_node_t *root){
    ...
    maxSumPath = MAX(MAX(valuetobeReturnedtoParent, recycleSum), maxSumPath);
    return valuetobeReturnedtoParent;
}
...
static void
_getDiameterBT_increment(int *currentD, int *lmax){
    *currentD += 1;
    if(*currentD > *lmax)
        *lmax = *currentD;
    return;
...
static void
_getDiameterBT_decrement(int *currentD, int *rmin){
    *currentD -= 1;
    if(*currentD < *rmin)
        *rmin = *currentD;
    return;
...
static tree_node_t*
_getDiameterBT(tree_node_t *root, int *lmax, int *rmin, int *currentD){

   if(root->left){
        _getDiameterBT_increment(currentD, lmax);
        _getDiameterBT(root->left, lmax, rmin, currentD);
        _getDiameterBT_decrement(currentD, rmin);
   }

   if(root->right){
       _getDiameterBT_decrement(currentD, rmin);
       _getDiameterBT(root->right, lmax, rmin, currentD);
       _getDiameterBT_increment(currentD, lmax);
   }
...
getDiameterBT(tree_t *tree){
    ...
    _getDiameterBT(tree->root, &lmax, &rmin, &currentD);
    return (lmax - rmin);
}
...
BFS(tree_t *tree, int lmax){
	...
	while(is_queue_empty(q) == Q_FALSE){
